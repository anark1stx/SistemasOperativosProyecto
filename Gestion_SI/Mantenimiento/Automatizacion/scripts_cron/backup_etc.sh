...
rsync -e "ssh -i /home/admin/.ssh/id_rsa -p 49555" -arz /etc admin@192.168.0.5:/backup/Linux/ &>/dev/null && echo "Directorio ETC respaldado con exito" >> /logs/respaldos.log || echo "Hubo errores respaldando directorio etc" >> /logs/respaldos.log
...
ssh admin@192.168.0.5 -i /home/admin/.ssh/id_rsa -p 49555 'cd /backup/Linux && sudo git add -A && sudo git commit -m "Respaldo ETC $1"'
