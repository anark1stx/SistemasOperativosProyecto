...
mysqldump -u root sibim -p$adminpwd --routines --add-drop-database > sibim.sql && echo "dump generado" || (echo $(date '+%d/%m/%Y %H:%M:%S'): "hubieron errores efecutando dump sibim" ; exit)
...
ssh admin@192.168.0.5 -i /home/admin/.ssh/id_rsa -p 49555 'cd /backup/SIBIM-BDS && sudo git add -A && sudo git commit -m "Respaldo BDS $1"'
