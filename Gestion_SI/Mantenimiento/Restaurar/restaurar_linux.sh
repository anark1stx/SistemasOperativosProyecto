...
cd /
echo "$(date '+%d/%m/%Y %H:%M:%S'): Restaurando Linux" >> /logs/respaldos.log
...
cp -R /var/Linux/home / && echo "(date '+%d/%m/%Y %H:%M:%S'): Directorio home restaurado con exito" >> /logs/respaldos.log || echo "(date '+%d/%m/%Y %H:%M:%S'): Hubo errores al sobreescribir el directorio home" >> /logs/respaldos.log
cp -R /var/Linux/etc / && echo "(date '+%d/%m/%Y %H:%M:%S'): Directorio etc restaurado con exito" >> /logs/respaldos.log || echo "(date '+%d/%m/%Y %H:%M:%S'): Hubo errores al sobreescribir el directorio etc" >> /logs/respaldos.log
rm -rf /var/Linux
